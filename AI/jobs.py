...
job_requirements = {"Degree": "Bio", "Age": 20, "Experience": 2, "faculty": "none", "Skills": ["Art", "Biology", "Programming"]}
...
def cost(candidate):
    # Define weights
    weights = {"skills": 4, "degree": 3, "experience": 2, "age": 3}
...
def get_neighbors(candidate, num_neighbors=10):
    if prioritize_degree == "n":
        neighbors = [c for c in candidates if len(set(c["Skills"]).intersection(set(job_requirements["Skills"]))) >= 2]
        if not neighbors:
            neighbors = [c for c in candidates if c["Degree"] in degree_to_faculty[candidate["Degree"]]]
    else:
        neighbors = [c for c in candidates if c["Degree"] in degree_to_faculty[candidate["Degree"]] and any(skill in c["Skills"] for skill in candidate["Skills"])]
