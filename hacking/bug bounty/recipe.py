...
def find_path(recipes, start_items, target):
    # Convert the recipe list into a dictionary for easy lookup
    recipe_dict = {}
    for item1, item2, result in recipes:
        if item1 not in recipe_dict:
            recipe_dict[item1] = {}
        if item2 not in recipe_dict:
            recipe_dict[item2] = {}
        recipe_dict[item1][item2] = result
        recipe_dict[item2][item1] = result

    # Use a queue to perform a BFS
    queue = deque([(item, [item]) for item in start_items])
    visited = set(start_items)  # Keep track of visited nodes
    while queue:
        current_item, path = queue.popleft()
        if current_item == target:
            return path
        for item in recipe_dict.get(current_item, {}):
            result = recipe_dict[current_item][item]
            if result not in visited:
                visited.add(result)
                queue.append((result, path + [result]))
...
start_items = ["Fire", "Water", "Earth", "Air"]
target = "XSS"

path = find_path(recipes, start_items, target)
if path is None:
    print("No path found")
else:
    print(" -> ".join(path))
